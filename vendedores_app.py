...
/* CORREÇÃO MOBILE: contraste dos filtros e campos */
...
/* Esconde elementos de branding/navegação do Streamlit quando possível */
#MainMenu,
header,
footer,
...
for row in f.itertuples():
    ...
    marca_txt = str(row.marca).replace('&','&amp;').replace('<','&lt;').replace('>','&gt;')
    card = (f'<div class="product-card">'
            f'<div><span class="badge {badge_class}">{origem_txt}</span>'
            f'<div class="desc">{desc}</div>'
            f'<div class="meta"><b>Marca:</b> {marca_txt}<br><b>Origem:</b> {origem_txt}</div></div>'
            f'<div class="stock-box"><div class="qty">✓ {qtd} {un}</div>'
            f'<div class="price">{brl(row.preco_unitario)}</div><div class="price-label">VALOR UNITÁRIO</div></div></div>')
    cards.append(card)

st.markdown('<div class="product-grid">' + ''.join(cards) + '</div>', unsafe_allow_html=True)
