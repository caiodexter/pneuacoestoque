...
/* ===== V6 - CHECKOUT INLINE CINZA (SEM MODAL ESCURO) ===== */
.st-key-cart_checkout_panel {
    background:#e5e7eb !important;
    border:1px solid #9ca3af !important;
    border-radius:18px !important;
...
.st-key-cart_checkout_panel *,
.st-key-cart_checkout_panel h1,
.st-key-cart_checkout_panel h2,
.st-key-cart_checkout_panel h3,
.st-key-cart_checkout_panel p,
.st-key-cart_checkout_panel label,
.st-key-cart_checkout_panel span,
.st-key-cart_checkout_panel small {
    color:#000000 !important;
    -webkit-text-fill-color:#000000 !important;
    opacity:1 !important;
}

.st-key-cart_checkout_panel input,
.st-key-cart_checkout_panel textarea,
.st-key-cart_checkout_panel [data-baseweb="input"],
.st-key-cart_checkout_panel [data-baseweb="textarea"] {
    background:#ffffff !important;
    color:#000000 !important;
    -webkit-text-fill-color:#000000 !important;
    border-color:#9ca3af !important;
...
.st-key-cart_checkout_panel div[data-testid="stNumberInput"] button {
    background:#d1d5db !important;
    color:#000000 !important;
    border-color:#9ca3af !important;
}
.st-key-cart_checkout_panel div[data-testid="stNumberInput"] button *,
.st-key-cart_checkout_panel div[data-testid="stNumberInput"] button svg {
    color:#000000 !important;
    fill:#000000 !important;
    -webkit-text-fill-color:#000000 !important;
}

.st-key-cart_checkout_panel hr {
    border-color:#9ca3af !important;
}

.st-key-cart_checkout_panel div[data-testid="stAlert"] {
    background:#f3f4f6 !important;
    border-color:#9ca3af !important;
}
.st-key-cart_checkout_panel div[data-testid="stAlert"] * {
...
/* Botão de finalizar */
.st-key-cart_checkout_panel button[kind="primary"] {
    background:#d1d5db !important;
    color:#000000 !important;
...
# V6: não usamos mais st.dialog no celular/PWA.
# O modal do Streamlit herda o tema escuro do iOS e ignora parte do CSS.
# Em vez disso, mostramos um painel cinza dentro da própria página.
if "mostrar_checkout_inline" not in st.session_state:
    st.session_state["mostrar_checkout_inline"] = False
...
if carrinho:
    with st.container(key="floating_cart"):
        ...
        if st.button(
            ...
            st.session_state["mostrar_checkout_inline"] = True
            st.rerun()

if st.session_state.get("mostrar_checkout_inline"):
    with st.container(key="cart_checkout_panel"):
        topo1, topo2 = st.columns([6,1])
        with topo1:
            st.markdown("## 🛒 Finalizar orçamento")
        with topo2:
            if st.button("✕", key="fechar_checkout_inline", help="Fechar"):
                st.session_state["mostrar_checkout_inline"] = False
                st.rerun()
