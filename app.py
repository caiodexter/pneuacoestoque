...
def dashboard():
    ...
    baixos = df[(df["quantidade"] > 0) & (df["quantidade"] <= 2)].copy()
    ...
    tabela = filtrado[[c for c in ["origem","marca","descricao","quantidade","preco_unitario","valor_estoque"] if c in filtrado.columns]].copy()
    ...
    with a2:
        st.markdown(f'<div class="alert-yellow">⚠️ {len(baixos)} produto(s) com apenas 1 ou 2 unidades.</div>',unsafe_allow_html=True)
        if st.button("Ver estoque baixo ❯",key="v6_low",use_container_width=True):
            st.session_state["v6_show_low"] = not st.session_state.get("v6_show_low",False)

    if st.session_state.get("v6_show_zero",False):
        z=zerados[[c for c in ["origem","marca","descricao","quantidade","preco_unitario"] if c in zerados.columns]]
        st.dataframe(z,hide_index=True,use_container_width=True,height=330)
    if st.session_state.get("v6_show_low",False):
        b=baixos[[c for c in ["origem","marca","descricao","quantidade","preco_unitario"] if c in baixos.columns]].sort_values("quantidade")
