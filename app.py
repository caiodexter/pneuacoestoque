...
def load_products(active_only=True):
    q = """
        SELECT id, origem, descricao, marca, preco_unitario, quantidade,
               preco_unitario * quantidade AS valor_estoque, ativo, COALESCE(estoque_minimo,2) AS estoque_minimo, imagem_url
        FROM produtos
    """
    if active_only:
        q += f" WHERE {ativo_sql()}"
        ...
        if not df_img.empty:
            ...
            if not base_img.empty:
                ...
                if st.button("💾 Salvar imagem", key="salvar_img", use_container_width=True):
                    with engine.begin() as conn:
                        conn.execute(
                            text("UPDATE produtos SET imagem_url=:url WHERE id=:id"),
                            {"url": url_img or None, "id": int(pid_img)}
                        )
                    try:
                        registrar_auditoria(
                            "ALTERAÇÃO DE IMAGEM",
                ...
                if atual and st.button("🗑️ Remover imagem", key="remover_img", use_container_width=True):
                    with engine.begin() as conn:
                        conn.execute(
                            text("UPDATE produtos SET imagem_url=NULL WHERE id=:id"),
                            {"id": int(pid_img)}
                        )
