...
def estoque():
    ...
    st.markdown("---")
    st.subheader("🤖 Automação total das imagens")
    st.caption(
        "Busca as fotos, baixa os arquivos, envia para o Supabase Storage e vincula "
        "cada foto ao ID do produto. Imagens CONFIRMADAS manualmente nunca são substituídas."
    )

    if st.session_state.get("perfil") in ["ADMINISTRADOR","GERENTE"]:
        df_auto = load_products()
        sem_img = df_auto[
            df_auto["imagem_url"].fillna("").astype(str).str.strip().eq("")
        ]
        aa1, aa2, aa3 = st.columns(3)
        aa1.metric("Produtos", len(df_auto))
        aa2.metric("Sem imagem", len(sem_img))
        aa3.metric("Com imagem", len(df_auto) - len(sem_img))

        sb_url, sb_key, bucket = _supabase_image_config()
        if not sb_url or not sb_key:
            st.warning(
                "Para eu armazenar as fotos no seu próprio Supabase, adicione nos Secrets: "
                "SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY. A chave não deve ser colocada no código."
            )
        else:
            st.success(f"Supabase Storage configurado • bucket: {bucket}")
            st.caption(
                "O processo gratuito usa busca de imagens e grava uma cópia no seu Storage, "
                "evitando depender para sempre do endereço externo."
            )
            if st.button(
                "🚀 BUSCAR + FAZER UPLOAD + VINCULAR TODAS AS FOTOS",
                type="primary",
                use_container_width=True,
                disabled=sem_img.empty,
                key="auto_total_img"
            ):
                barra_total = st.progress(0)
                msg_total = st.empty()
                try:
                    # Processa em blocos para a interface sobreviver melhor a reruns/timeouts.
                    resultado_total = preencher_todas_imagens_automaticamente(
                        limite=25, score_minimo=35
                    )
                    barra_total.progress(1.0)
                    qtd_ok = int((resultado_total["status"] == "AUTO").sum()) if not resultado_total.empty else 0
                    msg_total.success(
                        f"Bloco concluído: {qtd_ok}/{len(resultado_total)} fotos armazenadas. "
                        "Clique novamente para continuar com os produtos restantes."
                    )
                    if not resultado_total.empty:
                        st.dataframe(resultado_total, use_container_width=True, hide_index=True)
                except Exception as e:
                    st.error(f"Não foi possível executar a automação: {e}")
