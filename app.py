...
def dashboard():
    ...
    <style>
      ...
      .block-container{max-width:1480px;padding-top:1.05rem;padding-left:1.4rem;padding-right:1.4rem}
      .dash-topline{display:flex;justify-content:space-between;align-items:flex-start;gap:20px;margin-bottom:16px}
      .dash-title{font-size:34px;font-weight:900;color:#17233a;line-height:1.05;letter-spacing:-.02em}
      .dash-sub{color:#68758a;font-size:14px;margin-top:7px}
      .dash-user{font-size:13px;font-weight:700;color:#334155;background:#fff;border:1px solid #dfe6ef;border-radius:12px;padding:10px 14px;white-space:nowrap;box-shadow:0 3px 10px rgba(15,23,42,.04)}
      .metric-card{background:#fff;border:1px solid #dfe6ef;border-radius:16px;min-height:132px;padding:18px 18px;text-align:left;box-shadow:0 4px 14px rgba(15,23,42,.055)}
      .metric-title{font-size:12px;font-weight:800;color:#4c5a70;text-transform:uppercase;min-height:20px}
      .metric-icon{display:inline-flex;align-items:center;justify-content:center;width:48px;height:48px;border-radius:13px;background:#eaf2ff;font-size:24px;margin:10px 0 8px}
      .metric-value{font-size:25px;font-weight:900;color:#13213b;line-height:1.1;white-space:nowrap}
      .metric-foot{font-size:12px;color:#16924c;font-weight:700;margin-top:7px}
      .panel{background:#fff;border:1px solid #dfe6ef;border-radius:16px;padding:0;box-shadow:0 4px 13px rgba(15,23,42,.045);margin-top:15px;overflow:hidden}
      .panel-title{font-size:17px;font-weight:900;color:#153056;margin:0;padding:15px 17px 3px}
      .panel-sub{font-size:12px;color:#6b778b;margin:0;padding:0 17px 13px;border-bottom:1px solid #e8edf3}
      .alert-red{background:#fff0f1;border:1px solid #ffb5ba;border-radius:12px;padding:15px;color:#a5222b;font-weight:800}
      .alert-yellow{background:#fff7e8;border:1px solid #ffd37a;border-radius:12px;padding:15px;color:#9a5c00;font-weight:800}
      ...
      .footer-box{background:transparent;color:#7a8698;border-top:1px solid #e5eaf0;padding:18px 4px;margin-top:22px;display:flex;justify-content:center;font-size:12px}
      section[data-testid="stSidebar"]{background:#fff;border-right:1px solid #e6ebf2}
      section[data-testid="stSidebar"] [role="radiogroup"] label{border-radius:10px;padding:7px 9px;margin:2px 0}
      section[data-testid="stSidebar"] [role="radiogroup"] label:has(input:checked){background:#0d2f5f;color:#fff}
      section[data-testid="stSidebar"] [role="radiogroup"] label:has(input:checked) p{color:#fff!important;font-weight:700}
    </style>
    """, unsafe_allow_html=True)
    ...
    cards = [
        (c1,"Valor total do estoque","💰",brl(valor_total),f"{qtd_total} unidades","#e8f2ff"),
        (c2,"Pneus com nota","🧾",brl(valor_origem("PNEUS COM NOTA")),f'{qtd_origem("PNEUS COM NOTA")} unidades',"#eef3ff"),
        (c3,"Pneus sem nota","📦",brl(valor_origem("PNEUS SEM NOTA")),f'{qtd_origem("PNEUS SEM NOTA")} unidades',"#fff4dc"),
        (c4,"Diesel Pneus","🚚",brl(valor_origem("DIESEL PNEUS")),f'{qtd_origem("DIESEL PNEUS")} unidades',"#e8f8ef"),
    ]
    for col,tit,ico,val,foot,bg in cards:
        with col:
            st.markdown(f"""<div class="metric-card"><div class="metric-title">{tit}</div>
            <div class="metric-icon" style="background:{bg}">{ico}</div><div class="metric-value">{val}</div>
    ...
    origin_cards = [
        (o1,"TODOS","📋",len(df),qtd_total),
        (o2,"PNEUS COM NOTA","📄",modelos_origem("PNEUS COM NOTA"),qtd_origem("PNEUS COM NOTA")),
        (o3,"PNEUS SEM NOTA","📦",modelos_origem("PNEUS SEM NOTA"),qtd_origem("PNEUS SEM NOTA")),
        (o4,"DIESEL PNEUS","🚚",modelos_origem("DIESEL PNEUS"),qtd_origem("DIESEL PNEUS")),
    ]
    for col,nome,ico,mods,units in origin_cards:
        with col:
            st.markdown(
                f"""<div style="padding:15px 16px;background:#fff;border:1px solid #e1e7ef;border-radius:14px;min-height:105px;">
                <div style="font-size:12px;font-weight:800;color:#5d6a7e;">{ico} {nome}</div>
                <div style="font-size:25px;font-weight:900;color:#17233a;margin-top:7px;">{mods} modelos</div>
                <div style="font-size:12px;font-weight:700;color:#16924c;margin-top:5px;">↑ {units} unidades</div>
                </div>""",
                unsafe_allow_html=True
            )
    ...
    f1,f2,f3 = st.columns([1,1.1,1.35])
    with f1:
    ...
    with f2:
        ...
        marca_sel = st.selectbox("Marca", ["Todas as marcas"] + marcas, key="dash_marca_filtro")
    with f3:
        busca = st.text_input("Pesquisar modelo / medida", placeholder="Ex.: 295/80R22.5, Pirelli, FR88...", key="dash_modelo_busca")

    filtrado = base if marca_sel == "Todas as marcas" else base[base["marca"].astype(str) == marca_sel]
    if busca.strip():
    ...
    # Painéis inferiores: Top 10, valor por origem e alertas
    left,mid,right = st.columns([1.15,1.05,1.1], gap="medium")

    with left:
        st.markdown('<div class="panel"><div class="panel-title">📊 Quantidade por marca — Top 10</div></div>', unsafe_allow_html=True)
        ...
        fig_bar.update_layout(height=330, margin=dict(l=5,r=10,t=10,b=10),
    ...
    with mid:
        st.markdown('<div class="panel"><div class="panel-title">🧮 Valor do estoque por origem</div></div>', unsafe_allow_html=True)
        por_origem = df.groupby("origem",as_index=False)["valor_estoque"].sum()
        fig_pie = px.pie(por_origem,names="origem",values="valor_estoque",hole=.58)
        fig_pie.update_traces(textposition="inside",textinfo="percent")
        fig_pie.update_layout(height=330,margin=dict(l=5,r=5,t=10,b=10),
                              legend_title_text="",paper_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig_pie,use_container_width=True)

    with right:
        st.markdown('<div class="panel"><div class="panel-title">🔔 Alertas de estoque</div></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="alert-red">🚨 {len(zerados)} produto(s) com estoque zerado.</div>', unsafe_allow_html=True)
        if st.button("Ver produtos zerados ❯",key="v10_zero",use_container_width=True):
            st.session_state["v10_show_zero"] = not st.session_state.get("v10_show_zero",False)

        st.markdown(f'<div class="alert-yellow" style="margin-top:10px;">⚠️ {len(baixos)} produto(s) estão no limite mínimo definido.</div>', unsafe_allow_html=True)
        if st.button("Ver produtos no limite ❯",key="v10_low",use_container_width=True):
            st.session_state["v10_show_low"] = not st.session_state.get("v10_show_low",False)

    if st.session_state.get("v10_show_zero",False):
    ...
    if st.session_state.get("v10_show_low",False):
        st.markdown("### ⚠️ Produtos no limite mínimo")
    ...
    st.markdown('<div class="footer-box">© 2026 PNEUAÇO Estoque. Todos os direitos reservados.</div>',unsafe_allow_html=True)
