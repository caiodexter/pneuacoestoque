...
def dashboard():
    ...
    <style>
      ...
      .dash-title{
        font-size:31px;font-weight:900;color:#15233b;line-height:1.05;
      }
      ...
      .metric-card{
        background:#fff;border:1px solid #e1e7ef;border-radius:15px;
        min-height:205px;padding:18px 12px;text-align:center;
        box-shadow:0 4px 12px rgba(15,23,42,.06);
      }
      .metric-title{font-size:13px;font-weight:800;color:#475569;min-height:34px;}
      .metric-icon{font-size:42px;margin:8px 0 7px;}
      .metric-value{
        font-size:30px;font-weight:900;color:#13213b;
        line-height:1.1;white-space:nowrap;
      }
      .metric-value-money{font-size:26px;}
      .metric-foot{font-size:13px;color:#64748b;margin-top:8px;}
      .approved-card{
        display:inline-flex;align-items:center;gap:18px;margin-top:14px;
        background:#fff;border:1px solid #e2e8f0;border-radius:12px;
        padding:12px 18px;box-shadow:0 3px 10px rgba(15,23,42,.05);
        color:#166534;font-size:14px;font-weight:700;
      }
      .approved-number{font-size:21px;font-weight:900;color:#166534;}
      .panel{
        background:#fff;border:1px solid #e2e8f0;border-radius:16px;
        padding:19px;box-shadow:0 4px 13px rgba(15,23,42,.045);
        margin-top:15px;
      }
      ...
      .alert-red{
        background:#fff1f2;border:1px solid #fda4af;border-radius:12px;
        padding:17px;color:#991b1b;font-weight:800;min-height:63px;
      }
      .alert-yellow{
        background:#fffbeb;border:1px solid #f6c453;border-radius:12px;
        padding:17px;color:#92400e;font-weight:800;min-height:63px;
      }
      div[data-testid="stButton"] button{
        border-radius:10px;min-height:39px;font-weight:700;
      }
      .footer-box{
        background:#11243f;color:#fff;border-radius:0;
        padding:18px 24px;margin-top:22px;
        display:flex;justify-content:space-between;font-size:13px;
      }
      @media(max-width:900px){
        .dash-topline{display:block}
        .dash-user{display:inline-block;margin-top:10px}
    ...
    usuario_atual = st.session_state.get("user", "Administrador")
    perfil_atual = st.session_state.get("perfil", "ADMINISTRADOR")

    st.markdown(
        f"""
        <div class="dash-topline">
          <div>
            <div class="dash-title">Painel Gerencial</div>
            <div class="dash-sub">Visão geral do estoque e movimentações</div>
          </div>
          <div class="dash-user">📅 {hoje_dt.strftime("%d/%m/%Y - %H:%M")} &nbsp;&nbsp; 👤 {usuario_atual}</div>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    # -------- Movimentações com compatibilidade de nomes antigos/novos --------
    try:
        mov = pd.read_sql_query(text("SELECT * FROM movimentacoes"), engine)
    except Exception:
        mov = pd.DataFrame()

    data_col_mov = next((c for c in ["data_hora","data_movimento","criado_em","data"] if c in mov.columns), None)
    qtd_col_mov = next((c for c in ["quantidade","qtd"] if c in mov.columns), None)

    if data_col_mov:
        mov[data_col_mov] = pd.to_datetime(mov[data_col_mov], errors="coerce")
        mov_hoje = mov[mov[data_col_mov].dt.date == hoje].copy()
    else:
        mov_hoje = pd.DataFrame()

    entradas_hoje = 0
    saidas_hoje = 0
    if not mov_hoje.empty and "tipo" in mov_hoje.columns and qtd_col_mov:
        tipos = mov_hoje["tipo"].fillna("").astype(str).str.upper()
        quant = pd.to_numeric(mov_hoje[qtd_col_mov], errors="coerce").fillna(0)
        entradas_hoje = int(quant[tipos.str.contains("ENTR", regex=False)].sum())
        saidas_hoje = int(quant[tipos.str.contains(r"SAI|SAÍ|RETIR", regex=True)].sum())

    qtd_prod = pd.to_numeric(produtos["quantidade"], errors="coerce").fillna(0) if not produtos.empty else pd.Series(dtype=float)
    zerados = produtos[qtd_prod <= 0].copy() if not produtos.empty else pd.DataFrame()
    baixos = produtos[(qtd_prod > 0) & (qtd_prod <= 2)].copy() if not produtos.empty else pd.DataFrame()
    valor_total = float(pd.to_numeric(produtos["valor_estoque"], errors="coerce").fillna(0).sum()) if not produtos.empty else 0.0

    # -------- Orçamentos --------
    pendentes = 0
    aprovados_mes = 0
    try:
        orc = pd.read_sql_query(text("SELECT * FROM orcamentos"), engine)
        if not orc.empty and "status" in orc.columns:
            status = orc["status"].fillna("").astype(str).str.upper()
            pendentes = int(status.isin(["PENDENTE_APROVACAO","AGUARDANDO_APROVACAO"]).sum())
            data_orc = next((c for c in ["criado_em","data_hora","data","data_emissao"] if c in orc.columns), None)
            if data_orc:
                d = pd.to_datetime(orc[data_orc], errors="coerce")
                aprovados_mes = int(((status=="APROVADO") &
                                     (d.dt.date>=inicio_mes) &
                                     (d.dt.date<=hoje)).sum())
            else:
    ...
    # -------- Cards --------
    c1,c2,c3,c4,c5,c6 = st.columns([1,1,1.55,1,1,1], gap="medium")
    dados_cards = [
        (c1,"Entradas hoje","📥",f"{entradas_hoje}","Quantidade",""),
        (c2,"Saídas hoje","📤",f"{saidas_hoje}","Quantidade",""),
        (c3,"Valor total em estoque","💰",brl(valor_total),"Valor total","money"),
        (c4,"Estoque baixo (1–2)","⚠️",f"{len(baixos)}","produtos",""),
        (c5,"Produtos zerados","🚨",f"{len(zerados)}","produtos",""),
        (c6,"Orçamentos pendentes","🕒",f"{pendentes}","aguardando",""),
    ]
    for col,tit,ico,val,foot,kind in dados_cards:
        with col:
            money_cls = " metric-value-money" if kind == "money" else ""
            st.markdown(
                f"""
                <div class="metric-card">
                  <div class="metric-title">{tit}</div>
                  <div class="metric-icon">{ico}</div>
                  <div class="metric-value{money_cls}">{val}</div>
    ...
    st.markdown(
        f'<div class="approved-card">✅ Orçamentos aprovados no mês <span class="approved-number">{aprovados_mes}</span></div>',
        unsafe_allow_html=True
    )

    # -------- Alertas clicáveis --------
    st.markdown('<div class="panel"><div class="panel-title">🔔 Alertas</div>'
                '<div class="panel-sub">Clique em um alerta para ver os produtos relacionados</div>',
                unsafe_allow_html=True)

    a1,a2 = st.columns(2, gap="medium")
    with a1:
        st.markdown(f'<div class="alert-red">🚨 &nbsp; {len(zerados)} produto(s) estão com estoque zerado.</div>',
                    unsafe_allow_html=True)
        if st.button("Clique para ver a lista de produtos zerados  ❯", key="dash_zero", use_container_width=True):
            st.session_state["dash_show_zero"] = not st.session_state.get("dash_show_zero", False)

    with a2:
        st.markdown(f'<div class="alert-yellow">⚠️ &nbsp; {len(baixos)} produto(s) estão com apenas 1 ou 2 unidades.</div>',
                    unsafe_allow_html=True)
        if st.button("Clique para ver a lista de estoque baixo  ❯", key="dash_low", use_container_width=True):
            st.session_state["dash_show_low"] = not st.session_state.get("dash_show_low", False)

    if st.session_state.get("dash_show_zero", False):
        st.markdown("#### 🚨 Produtos zerados")
        if zerados.empty:
            st.success("Nenhum produto zerado.")
        else:
            z = zerados[[c for c in ["origem","marca","descricao","quantidade","preco_unitario"] if c in zerados.columns]].copy()
            st.dataframe(z, hide_index=True, use_container_width=True, height=360,
                         column_config={"preco_unitario":st.column_config.NumberColumn("Preço",format="R$ %.2f")})

    if st.session_state.get("dash_show_low", False):
        st.markdown("#### ⚠️ Produtos com estoque baixo")
        if baixos.empty:
            st.success("Nenhum produto com estoque baixo.")
        else:
            b = baixos[[c for c in ["origem","marca","descricao","quantidade","preco_unitario"] if c in baixos.columns]].copy()
            if "quantidade" in b.columns:
                b = b.sort_values("quantidade")
            st.dataframe(b, hide_index=True, use_container_width=True, height=420,
                         column_config={"preco_unitario":st.column_config.NumberColumn("Preço",format="R$ %.2f")})
    st.markdown("</div>", unsafe_allow_html=True)

    # -------- Parte inferior: movimentações + gráfico origem --------
    left,right = st.columns([1.35,1], gap="medium")

    with left:
        st.markdown('<div class="panel"><div class="panel-title">🧾 Movimentações recentes</div>', unsafe_allow_html=True)
        ...
        if recente.empty:
            ...
            if data_col_mov:
                recente = recente.sort_values(data_col_mov, ascending=False).head(7)

            # Anexa descrição/origem usando produto_id
            if "produto_id" in recente.columns:
                try:
                    mapa_prod = produtos[["id","descricao","origem"]].rename(columns={"id":"produto_id"})
                    recente = recente.merge(mapa_prod, on="produto_id", how="left", suffixes=("","_produto"))
                except Exception:
                    pass

            ex = pd.DataFrame(index=recente.index)
            if data_col_mov:
                ex["Data/Hora"] = pd.to_datetime(recente[data_col_mov], errors="coerce").dt.strftime("%d/%m/%Y %H:%M")
            if "tipo" in recente.columns:
                ex["Tipo"] = recente["tipo"]
            if "descricao" in recente.columns:
                ex["Pneu"] = recente["descricao"]
            if "origem" in recente.columns:
                ex["Origem"] = recente["origem"]
            if qtd_col_mov:
                ex["Quantidade"] = recente[qtd_col_mov]
            if "usuario" in recente.columns:
                ex["Usuário"] = recente["usuario"]

            st.dataframe(ex, hide_index=True, use_container_width=True, height=300)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with right:
        ...
        if produtos.empty:
            ...
            por_origem = (produtos.groupby("origem", as_index=False)["valor_estoque"]
                          .sum().sort_values("valor_estoque", ascending=False))
            fig = px.pie(
                por_origem,
                names="origem",
    ...
    st.markdown(
        '<div class="footer-box"><span><b>PNEUAÇO Estoque</b> — Sistema de Controle de Estoque</span>'
        '<span>Dashboard Gerencial</span><span>Controle e rastreabilidade</span></div>',
        unsafe_allow_html=True
    )
