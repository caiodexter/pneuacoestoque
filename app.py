...
def dashboard():
    ...
    <style>
      ...
      .pg-sub{color:#64748b;font-size:14px;margin-top:-8px}
      .pg-card{
        background:#fff;border:1px solid #e2e8f0;border-radius:16px;
        padding:18px 14px;min-height:185px;text-align:center;
        box-shadow:0 4px 14px rgba(15,23,42,.06)
      }
      .pg-title{font-size:14px;font-weight:700;color:#334155;margin-bottom:12px}
      .pg-icon{font-size:42px;line-height:1;margin:8px 0}
      .pg-value{font-size:32px;font-weight:800;color:#0f172a;white-space:nowrap}
      .pg-foot{font-size:13px;color:#64748b;margin-top:6px}
      .pg-approved{
        display:inline-block;background:#fff;border:1px solid #e2e8f0;border-radius:12px;
        padding:12px 18px;box-shadow:0 3px 10px rgba(15,23,42,.05);margin:16px 0 8px 0
      }
      .pg-panel{
        background:#fff;border:1px solid #e2e8f0;border-radius:16px;
        padding:18px;box-shadow:0 4px 14px rgba(15,23,42,.05);margin-top:12px
      }
      .pg-alert-red{
        background:#fff1f2;border:1px solid #fecdd3;border-radius:14px;
        padding:16px 18px;color:#991b1b;font-weight:700
      }
      .pg-alert-orange{
        background:#fffbeb;border:1px solid #fde68a;border-radius:14px;
        padding:16px 18px;color:#92400e;font-weight:700
      }
      div.stButton>button{
        border-radius:11px;min-height:42px;font-weight:700;border:1px solid #dbe3ef
      }
    </style>
    """, unsafe_allow_html=True)

    st.title("🏠 Painel Gerencial")
    st.markdown('<div class="pg-sub">Visão geral do estoque e movimentações</div>', unsafe_allow_html=True)
    ...
    qtd_num = pd.to_numeric(produtos_ger["quantidade"], errors="coerce").fillna(0) if not produtos_ger.empty else pd.Series(dtype=float)
    zerados_df = produtos_ger[qtd_num <= 0].copy() if not produtos_ger.empty else pd.DataFrame()
    baixo_df = produtos_ger[(qtd_num > 0) & (qtd_num <= 2)].copy() if not produtos_ger.empty else pd.DataFrame()
    estoque_zerado = len(zerados_df)
    ...
    cards = [
        (c1,"Entradas hoje","📥",entradas_hoje,"Quantidade"),
        (c2,"Saídas hoje","📤",saidas_hoje,"Quantidade"),
        (c3,"Valor total em estoque","💰",brl(valor_total),"Valor total"),
        (c4,"Estoque baixo (1–2)","⚠️",estoque_baixo,"produtos"),
        (c5,"Produtos zerados","🚨",estoque_zerado,"produtos"),
        (c6,"Orçamentos pendentes","🕒",pendentes,"aguardando"),
    ]
    for col,titulo,icone,valor,rodape in cards:
        with col:
            st.markdown(f"""
            <div class="pg-card">
              <div class="pg-title">{titulo}</div>
              <div class="pg-icon">{icone}</div>
              <div class="pg-value">{valor}</div>
              <div class="pg-foot">{rodape}</div>
            </div>""", unsafe_allow_html=True)

    st.markdown(f'<div class="pg-approved">✅ <b>Orçamentos aprovados no mês</b>&nbsp;&nbsp;&nbsp;<b style="font-size:20px">{aprovados_mes}</b></div>', unsafe_allow_html=True)

    st.markdown('<div class="pg-panel">', unsafe_allow_html=True)
    st.subheader("🔔 Alertas")
    st.caption("Clique em um alerta para ver os produtos relacionados")

    a1,a2 = st.columns(2)
    with a1:
        st.markdown(f'<div class="pg-alert-red">🚨 {estoque_zerado} produto(s) estão com estoque zerado.</div>', unsafe_allow_html=True)
        if st.button("Clique para ver a lista de produtos zerados  ›", key="show_zero", use_container_width=True):
            st.session_state["show_zero"] = not st.session_state.get("show_zero", False)
    with a2:
        st.markdown(f'<div class="pg-alert-orange">⚠️ {estoque_baixo} produto(s) estão com apenas 1 ou 2 unidades.</div>', unsafe_allow_html=True)
        if st.button("Clique para ver a lista de estoque baixo  ›", key="show_low", use_container_width=True):
            st.session_state["show_low"] = not st.session_state.get("show_low", False)

    if st.session_state.get("show_zero", False):
        st.markdown("#### 🚨 Produtos zerados")
        if zerados_df.empty:
            st.success("Nenhum produto zerado.")
        else:
            zcols = [c for c in ["origem","marca","descricao","quantidade","preco_unitario"] if c in zerados_df.columns]
            st.dataframe(
                zerados_df[zcols], hide_index=True, use_container_width=True, height=360,
                column_config={"preco_unitario": st.column_config.NumberColumn("Preço", format="R$ %.2f")}
            )

    if st.session_state.get("show_low", False):
        st.markdown("#### ⚠️ Produtos com estoque baixo")
        if baixo_df.empty:
            st.success("Nenhum produto com estoque baixo.")
        else:
            bcols = [c for c in ["origem","marca","descricao","quantidade","preco_unitario"] if c in baixo_df.columns]
            st.dataframe(
                baixo_df[bcols].sort_values("quantidade"), hide_index=True, use_container_width=True, height=420,
                column_config={"preco_unitario": st.column_config.NumberColumn("Preço", format="R$ %.2f")}
            )
    ...
    if mov_ger.empty:
        ...
        recente = mov_ger.copy()
        if "data_hora" in recente.columns:
            recente = recente.sort_values("data_hora", ascending=False).head(8)
            recente["data_hora"] = recente["data_hora"].dt.strftime("%d/%m/%Y %H:%M")
        keep = [c for c in ["data_hora","tipo","descricao","origem","quantidade","qtd","usuario"] if c in recente.columns]
        st.dataframe(recente[keep], hide_index=True, use_container_width=True, height=300)
    st.markdown("</div>", unsafe_allow_html=True)
