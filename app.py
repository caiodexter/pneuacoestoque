...
def historico_auditoria():
    ...
    st.caption("Alterações de preço e movimentações de estoque, com usuário, data e detalhes.")
    hist = pd.read_sql_query(text("""
        SELECT id,data_hora,usuario,tipo,produto_id,descricao,origem,
               valor_anterior,valor_novo,quantidade_anterior,quantidade_movimentada,
               quantidade_nova,nf,destino,motivo,observacao
        FROM auditoria ORDER BY data_hora DESC,id DESC
    """), engine)
    if hist.empty:
        st.info("Ainda não existem registros. As próximas alterações passarão a aparecer aqui.")
        return

    c1,c2,c3 = st.columns(3)
    with c1: tipos = st.multiselect("Tipo de operação", sorted(hist["tipo"].dropna().unique()))
    with c2: usuarios_hist = st.multiselect("Usuário", sorted(hist["usuario"].dropna().unique()))
    with c3: origens_hist = st.multiselect("Origem", sorted(hist["origem"].dropna().unique()))
    busca = st.text_input("Pesquisar pneu", placeholder="Digite medida, marca ou descrição...").strip()

    f=hist.copy()
    if tipos: f=f[f["tipo"].isin(tipos)]
    if usuarios_hist: f=f[f["usuario"].isin(usuarios_hist)]
    if origens_hist: f=f[f["origem"].isin(origens_hist)]
    if busca: f=f[f["descricao"].fillna("").str.contains(busca,case=False,na=False)]

    k1,k2,k3=st.columns(3)
    k1.metric("Registros",len(f))
    k2.metric("Alterações de preço",int(f["tipo"].eq("ALTERAÇÃO DE PREÇO").sum()))
    k3.metric("Movimentações",int(f["tipo"].str.startswith("MOVIMENTAÇÃO",na=False).sum()))

    exib=f.rename(columns={"data_hora":"DATA / HORA","usuario":"USUÁRIO","tipo":"OPERAÇÃO",
        "produto_id":"ID","descricao":"PNEU / DESCRIÇÃO","origem":"ORIGEM",
        "valor_anterior":"VALOR ANTERIOR","valor_novo":"VALOR NOVO",
        "quantidade_anterior":"QTD. ANTERIOR","quantidade_movimentada":"QTD. MOVIMENTADA",
        "quantidade_nova":"QTD. FINAL","nf":"NF","destino":"DESTINO",
        "motivo":"MOTIVO","observacao":"OBSERVAÇÃO"})
    st.dataframe(exib,hide_index=True,use_container_width=True,height=560,
        column_config={"VALOR ANTERIOR":st.column_config.NumberColumn("VALOR ANTERIOR",format="R$ %.2f"),
                       "VALOR NOVO":st.column_config.NumberColumn("VALOR NOVO",format="R$ %.2f")})

    output=io.BytesIO()
    with pd.ExcelWriter(output,engine="openpyxl") as writer:
        exib.to_excel(writer,index=False,sheet_name="Historico")
    st.download_button("⬇️ Exportar histórico filtrado",data=output.getvalue(),
        file_name=f"historico_auditoria_{datetime.now():%Y%m%d_%H%M}.xlsx",
