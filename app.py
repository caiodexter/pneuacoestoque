...
def _normalizar_img_txt(valor):
    import re as _re
    import unicodedata as _ud
    s = "" if valor is None else str(valor)
    s = _ud.normalize("NFKD", s).encode("ascii", "ignore").decode("ascii").upper()
    s = _re.sub(r"[^A-Z0-9./-]+", " ", s)
    return " ".join(s.split())

def _tokens_importantes_imagem(marca, descricao):
    txt = _normalizar_img_txt(f"{marca} {descricao}")
    ignorar = {
        "PNEU","TL","TT","PR","R1","R-1","R2","R-2","RADIAL","DIAGONAL",
        "COM","SEM","NOTA","RODOU","NO","PATIO"
    }
    tokens = []
    for t in txt.split():
        if len(t) >= 3 and t not in ignorar:
            tokens.append(t)
    return list(dict.fromkeys(tokens))

def _pontuar_candidato_imagem(produto, resultado):
    marca = _normalizar_img_txt(produto.get("marca", ""))
    descricao = _normalizar_img_txt(produto.get("descricao", ""))
    alvo = f"{marca} {descricao}"
    texto = _normalizar_img_txt(
        f'{resultado.get("title","")} {resultado.get("url","")} '
        f'{resultado.get("image","")} {resultado.get("source","")}'
    )
    score = 0
    if marca and marca in texto:
        score += 35

    tokens = _tokens_importantes_imagem(marca, descricao)
    for token in tokens:
        if token in texto:
            score += 8

    # Valoriza medida/modelo com números.
    numericos = [t for t in tokens if any(c.isdigit() for c in t)]
    for token in numericos:
        if token in texto:
            score += 10

    # Penaliza fontes tipicamente pouco confiáveis para foto de catálogo.
    baixo = texto.lower()
    for termo in ("facebook", "pinterest", "instagram", "aliexpress"):
        if termo in baixo:
            score -= 12

    return max(score, 0)

def buscar_imagem_automatica(produto, max_results=8):
    marca = str(produto.get("marca", "") or "").strip()
    descricao = str(produto.get("descricao", "") or "").strip()
    consulta = f'{marca} {descricao} tire pneu'
    ...
    for r in resultados or []:
        ...
        if not url or not str(url).startswith(("http://", "https://")):
            continue
        score = _pontuar_candidato_imagem(produto, r)
        candidatos.append({
            "url": url,
            "fonte": r.get("url") or r.get("source") or "",
...
def _supabase_image_config():
    """Lê configuração do Storage sem expor as chaves na tela."""
    try:
        sb_url = str(st.secrets["SUPABASE_URL"]).rstrip("/")
        sb_key = str(st.secrets["SUPABASE_SERVICE_ROLE_KEY"])
        bucket = str(st.secrets.get("SUPABASE_IMAGE_BUCKET", "pneus"))
        return sb_url, sb_key, bucket
    except Exception:
        return None, None, "pneus"

def _baixar_imagem(url, timeout=20):
    headers = {
        "User-Agent": "Mozilla/5.0 (compatible; PneuacoImageBot/1.0)",
...
def _ext_por_content_type(ctype):
    mapa = {
        "image/jpeg": ".jpg", "image/jpg": ".jpg", "image/png": ".png",
        "image/webp": ".webp", "image/gif": ".gif", "image/avif": ".avif"
    }
    return mapa.get(ctype, ".jpg")

def _garantir_bucket_supabase(sb_url, sb_key, bucket):
    headers = {"Authorization": f"Bearer {sb_key}", "apikey": sb_key, "Content-Type": "application/json"}
    # Create is idempotent enough for our flow; 400/409 usually means it already exists.
    r = requests.post(
    ...
    if r.status_code not in (200, 201, 400, 409):
        raise RuntimeError(f"Falha ao preparar bucket: HTTP {r.status_code} - {r.text[:180]}")

def _upload_supabase_imagem(sb_url, sb_key, bucket, produto_id, image_bytes, ctype):
    ext = _ext_por_content_type(ctype)
    digest = hashlib.sha1(image_bytes).hexdigest()[:12]
    caminho = f"produtos/{int(produto_id)}/{digest}{ext}"
    headers = {
    ...
    r = requests.post(
        f"{sb_url}/storage/v1/object/{bucket}/{caminho}",
        headers=headers,
        data=image_bytes,
        timeout=30,
    )
    if r.status_code not in (200, 201):
        raise RuntimeError(f"Upload falhou: HTTP {r.status_code} - {r.text[:180]}")
    return f"{sb_url}/storage/v1/object/public/{bucket}/{caminho}"

def preencher_todas_imagens_automaticamente(limite=None, score_minimo=35):
    sb_url, sb_key, bucket = _supabase_image_config()
    if not sb_url or not sb_key:
        raise RuntimeError(
            "Faltam SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY nos Secrets do Streamlit."
        )
    _garantir_bucket_supabase(sb_url, sb_key, bucket)

    produtos = load_products()
    pend = produtos[
        produtos["imagem_url"].fillna("").astype(str).str.strip().eq("") &
        produtos["imagem_status"].fillna("PENDENTE").astype(str).ne("CONFIRMADA")
    ].copy()
    if limite:
        pend = pend.head(int(limite))

    resultados = []
    for _, prod in pend.iterrows():
        item = {
            "id": int(prod["id"]), "descricao": str(prod["descricao"]),
            "status": "SEM RESULTADO", "score": 0, "fonte": ""
        }
        try:
            candidatos = buscar_imagem_automatica(prod, max_results=10)
            # Tenta candidatos em ordem até achar uma URL realmente baixável.
            for cand in candidatos[:6]:
                if int(cand["score"]) < int(score_minimo):
                    continue
                try:
                    blob, ctype = _baixar_imagem(cand["url"])
                    public_url = _upload_supabase_imagem(
                        sb_url, sb_key, bucket, int(prod["id"]), blob, ctype
                    )
                        ...
                        conn.execute(
                            text("""
                                ...
                                SET imagem_url=:url,
                                    ...
                                    imagem_status='AUTO'
                                WHERE id=:id
                                  AND COALESCE(imagem_status,'PENDENTE') <> 'CONFIRMADA'
                            """),
                            {
                                "url": public_url,
                                "fonte": cand["fonte"] or cand["titulo"],
                                "id": int(prod["id"]),
                            }
                        )
                    item.update(
                        status="AUTO", score=int(cand["score"]),
                        fonte=cand["fonte"] or cand["titulo"]
                    )
                    break
                except Exception:
            ...
            item["status"] = "ERRO"
            item["fonte"] = str(e)[:160]
        resultados.append(item)
    return pd.DataFrame(resultados)



IMAGENS_VALIDADAS_PNEUACO = {
    512: {
        "descricao": "PIRELLI RA45 7.50-16 10PR TT",
        "imagem": "https://www.bellenzier.com.br/media/catalog/product/cache/1/image/800x/9df78eab33525d08d6e5fb8d27136e95/p/n/pneu-750-16-ra45-10pr-agricola-pirelli.png",
        "fonte": "https://www.bellenzier.com.br/pneu-7-50-16-ra45-i-1-10pr-tt-aro-16-agricola-pirelli.html",
        "confianca": "ALTA"
    },
    514: {
        "descricao": "PIRELLI TM95 9.5-24 6PR TT",
        "imagem": "https://www.bellenzier.com.br/media/tmp/webp/catalog/product/cache/1/image/9df78eab33525d08d6e5fb8d27136e95/p/n/pneu-tm95-agricola-trator-1_png.webp",
        "fonte": "https://www.bellenzier.com.br/pneu-agricola-9-5-24-tm95-r-1-06pr-tt-aro-24.html",
        "confianca": "ALTA"
    },
    516: {
        "descricao": "FORERUNNER SKS-1 10-16.5 12PR",
        "imagem": "https://cdn11.bigcommerce.com/s-e8i94i2k1a/images/stencil/1280x1280/products/175957/1487693/forerunner-sks-1-b-n1551664-aaa1-2__44270.1737132444.jpg?c=2",
        "fonte": "https://www.tiremart.com/forerunner-sks-1-10-16-5-12-ply-as-a-s-all-season-tire/",
        "confianca": "ALTA"
    },
    521: {
        "descricao": "PETLAS TA60 11.2-24 8PR",
        "imagem": "https://backend.prioritytire.com/media/catalog/product/cache/89bfd9074608e40851f4cbb9f524ef74/tires/petlas/petlas-ta-60-b-aaa-1.jpg",
        "fonte": "https://www.prioritytire.com/by-brand/petlas-tires/ta-60/11-2-24-116a6-8-ply-rear-68288",
        "confianca": "ALTA"
    },
}

def aplicar_pacote_validado_pneuaco():
    sb_url, sb_key, bucket = _supabase_image_config()
    if not sb_url or not sb_key:
        raise RuntimeError("Supabase Storage não configurado.")
    _garantir_bucket_supabase(sb_url, sb_key, bucket)

    linhas = []
    for produto_id, item in IMAGENS_VALIDADAS_PNEUACO.items():
        status = "ERRO"
        detalhe = ""
        try:
            with engine.begin() as conn:
                atual = conn.execute(
                    text("SELECT imagem_status, imagem_url FROM produtos WHERE id=:id"),
                    {"id": produto_id}
                ).mappings().first()
            if not atual:
                status, detalhe = "IGNORADO", "ID não encontrado"
            elif str(atual.get("imagem_status") or "").upper() == "CONFIRMADA":
                status, detalhe = "PROTEGIDA", "Imagem manual confirmada preservada"
            else:
                blob, ctype = _baixar_imagem(item["imagem"])
                public_url = _upload_supabase_imagem(
                    sb_url, sb_key, bucket, produto_id, blob, ctype
                )
                with engine.begin() as conn:
                    conn.execute(
                        text("""
                ...
                status, detalhe = "OK", item["confianca"]
        except Exception as e:
            detalhe = str(e)[:180]
        linhas.append({
            "id": produto_id,
...
def init_db():
    ...
    if dialect == "postgresql":
        ...
        with engine.begin() as conn:
            ...
            conn.execute(text("""
                ...
                ALTER TABLE produtos
                ADD COLUMN IF NOT EXISTS imagem_status TEXT DEFAULT 'PENDENTE' 
            """))
        return
    ...
    with engine.begin() as conn:
        ...
        if "imagem_url" not in nomes:
            conn.execute(text("ALTER TABLE produtos ADD COLUMN imagem_url TEXT"))
        if "imagem_fonte" not in nomes:
            conn.execute(text("ALTER TABLE produtos ADD COLUMN imagem_fonte TEXT"))
        if "imagem_status" not in nomes:
            conn.execute(text("ALTER TABLE produtos ADD COLUMN imagem_status TEXT DEFAULT 'PENDENTE'"))
...
def load_products(active_only=True):
    q = """
        SELECT id, origem, descricao, marca, preco_unitario, quantidade,
               preco_unitario * quantidade AS valor_estoque, ativo, COALESCE(estoque_minimo,2) AS estoque_minimo, imagem_url, imagem_fonte, COALESCE(imagem_status,'PENDENTE') AS imagem_status
        FROM produtos
    """
    if active_only:
        q += f" WHERE {ativo_sql()}"
...
def dashboard():
    ...
    tabela = filtrado[[c for c in ["imagem_url","origem","marca","descricao","quantidade","estoque_minimo","preco_unitario","valor_estoque"] if c in filtrado.columns]].copy()
    ...
    st.dataframe(
        ...
        column_config={"imagem_url": st.column_config.ImageColumn("IMAGEM", width="small"),
...
def estoque():
    ...
    st.markdown("---")
    st.subheader("✏️ Alterar descrição do produto")
    st.caption("Selecione o produto, confira a descrição atual e informe a nova descrição.")

    perfil_descricao = str(st.session_state.get("perfil", "")).upper()
    if perfil_descricao not in ("ADMINISTRADOR", "GERENTE"):
        st.info("A alteração da descrição está disponível somente para ADMINISTRADOR ou GERENTE.")
    else:
        todos_desc = load_products()
        origem_desc = st.selectbox(
            "Filtrar origem para alteração da descrição",
            ["TODAS","PNEUS COM NOTA","PNEUS SEM NOTA","DIESEL PNEUS"],
            key="descricao_origem"
        )
        base_desc = todos_desc if origem_desc == "TODAS" else todos_desc[todos_desc["origem"] == origem_desc]

        busca_desc = st.text_input(
            "Pesquisar pneu para alterar descrição",
            placeholder="Digite medida, marca ou parte da descrição...",
            key="descricao_busca"
        ).strip()

        if busca_desc and not base_desc.empty:
            m = (
                base_desc["descricao"].fillna("").astype(str).str.contains(busca_desc, case=False, na=False)
                | base_desc["marca"].fillna("").astype(str).str.contains(busca_desc, case=False, na=False)
            )
            base_desc = base_desc[m]

        if base_desc.empty:
            st.warning("Nenhum pneu encontrado para esse filtro.")
        else:
            opcoes_desc = {
                f"{int(r.id)} — {r.descricao} | {r.origem}": int(r.id)
                for r in base_desc.itertuples()
            }
            produto_desc_label = st.selectbox(
                "Produto para editar",
                list(opcoes_desc.keys()),
                key="descricao_produto"
            )
            produto_desc_id = opcoes_desc[produto_desc_label]
            produto_desc_row = todos_desc[todos_desc["id"] == produto_desc_id].iloc[0]
            descricao_anterior = str(produto_desc_row["descricao"] or "").strip()

            nova_descricao = st.text_input(
                "Nova descrição",
                value=descricao_anterior,
                key=f"nova_descricao_{produto_desc_id}"
            ).strip()

            confirmar_desc = st.checkbox(
                f"Confirmo a alteração da descrição do produto ID {produto_desc_id}.",
                key=f"confirmar_descricao_{produto_desc_id}"
            )

            if st.button(
                "💾 Salvar nova descrição",
                type="primary",
                disabled=not confirmar_desc,
                key=f"salvar_descricao_{produto_desc_id}"
            ):
                if not nova_descricao:
                    st.error("A descrição não pode ficar vazia.")
                elif nova_descricao == descricao_anterior:
                    st.info("A nova descrição é igual à descrição atual. Nenhuma alteração foi feita.")
                else:
                    with engine.begin() as conn:
                        conn.execute(
                            text("""
                                ...
                                WHERE id=:id
                            """),
                            {"nova": nova_descricao, "id": int(produto_desc_id)}
                        )
                    try:
                        registrar_auditoria(
                            tipo="ALTERAÇÃO DE DESCRIÇÃO",
                            produto_id=int(produto_desc_id),
                            descricao=nova_descricao,
                            origem=str(produto_desc_row["origem"]),
                            motivo="Alteração manual de descrição",
                            observacao=f"Descrição anterior: {descricao_anterior} | Nova descrição: {nova_descricao}"
                        )
                    except Exception:
                        pass
    ...
    st.markdown("---")
    st.subheader("🗑️ Excluir cadastro de produto")
    st.caption("Remove o produto das telas e do estoque ativo, preservando o histórico de movimentações e auditoria.")

    perfil_exclusao = str(st.session_state.get("perfil", "")).upper()
    if perfil_exclusao not in ("ADMINISTRADOR", "GERENTE"):
        st.info("A exclusão de cadastro está disponível somente para ADMINISTRADOR ou GERENTE.")
    else:
        todos_exclusao = load_products()
        origem_exclusao = st.selectbox(
            "Filtrar origem para exclusão",
            ["TODAS","PNEUS COM NOTA","PNEUS SEM NOTA","DIESEL PNEUS"],
            key="exclusao_origem"
        )
        base_exclusao = (
            todos_exclusao
            if origem_exclusao == "TODAS"
            else todos_exclusao[todos_exclusao["origem"] == origem_exclusao]
        )

        busca_exclusao = st.text_input(
            "Pesquisar produto para excluir",
            placeholder="Digite ID, medida, marca ou descrição...",
            key="exclusao_busca"
        ).strip()

        if busca_exclusao and not base_exclusao.empty:
            termo = busca_exclusao.lower()
            base_exclusao = base_exclusao[
                base_exclusao["descricao"].fillna("").astype(str).str.lower().str.contains(termo, regex=False)
                | base_exclusao["marca"].fillna("").astype(str).str.lower().str.contains(termo, regex=False)
                | base_exclusao["id"].astype(str).str.contains(termo, regex=False)
            ]

        if base_exclusao.empty:
            st.warning("Nenhum produto encontrado para exclusão.")
        else:
            opcoes_exclusao = {
                f'{int(r.id)} — {r.descricao} | {r.origem} | Estoque: {int(r.quantidade)}': int(r.id)
                for r in base_exclusao.itertuples()
            }
            produto_exclusao_label = st.selectbox(
                "Produto a excluir",
                list(opcoes_exclusao.keys()),
                key="produto_exclusao"
            )
            produto_exclusao_id = opcoes_exclusao[produto_exclusao_label]
            linha_exclusao = todos_exclusao[todos_exclusao["id"] == produto_exclusao_id].iloc[0]

            st.warning(
                f'Você está prestes a excluir o cadastro ID {produto_exclusao_id}: '
                f'{linha_exclusao["descricao"]}. Esta ação retira o produto do estoque ativo.'
            )
            confirmacao_exclusao = st.text_input(
                'Para confirmar, digite EXCLUIR',
                key=f"confirmar_exclusao_{produto_exclusao_id}"
            ).strip().upper()

            if st.button(
                "🗑️ Excluir cadastro",
                type="primary",
                disabled=confirmacao_exclusao != "EXCLUIR",
                key=f"excluir_produto_{produto_exclusao_id}"
            ):
                with engine.begin() as conn:
                    conn.execute(
                        text("""
                        ...
                        """),
                        {
                            "ativo": False if is_online_db() else 0,
                            "id": int(produto_exclusao_id)
                        }
                    )
                ...
                try:
                    registrar_auditoria(
                        ...
                        produto_id=int(produto_exclusao_id),
                        descricao=str(linha_exclusao["descricao"]),
                        origem=str(linha_exclusao["origem"]),
                        quantidade_anterior=int(linha_exclusao["quantidade"] or 0),
                        quantidade_nova=0,
                        motivo="Produto removido do estoque ativo",
                        observacao="Exclusão lógica: cadastro preservado no banco para manter histórico e rastreabilidade."
                    )
                except Exception:
                    pass

                st.success("Cadastro excluído do estoque ativo com sucesso.")
                st.rerun()
    ...
    st.markdown("---")
    st.subheader("📦 Pacote de imagens validadas")
    st.caption(
        "V15 remove a busca DuckDuckGo que falhou no Streamlit. "
        "Este botão trabalha somente com correspondências pesquisadas e validadas fora do app, "
        "faz a cópia para o Supabase Storage e preserva qualquer imagem CONFIRMADA manualmente."
    )

    if st.session_state.get("perfil") in ["ADMINISTRADOR","GERENTE"]:
        sb_url, sb_key, bucket = _supabase_image_config()
        p1, p2, p3 = st.columns(3)
        p1.metric("Pacote atual", len(IMAGENS_VALIDADAS_PNEUACO))
        p2.metric("Confiança", "ALTA")
        p3.metric("Destino", bucket)

        if not sb_url or not sb_key:
            st.error("Supabase Storage não configurado.")
        else:
            st.success(f"Supabase Storage configurado • bucket: {bucket}")
            if st.button(
                "⬆️ APLICAR PACOTE VALIDADO AO BANCO",
                type="primary",
                use_container_width=True,
                key="aplicar_pacote_validado"
            ):
                try:
                    resultado = aplicar_pacote_validado_pneuaco()
                    ok = int((resultado["resultado"] == "OK").sum())
                    st.success(f"Pacote processado: {ok}/{len(resultado)} imagens gravadas.")
                    st.dataframe(resultado, use_container_width=True, hide_index=True)
                    st.cache_data.clear()
                except Exception as e:
                    st.error(f"Falha ao aplicar pacote: {e}")

    st.subheader("🖼️ Gerenciar imagem do produto")
    st.caption("Associe uma imagem pública a cada pneu. Essa mesma imagem poderá ser usada na Dashboard, no app dos vendedores e no PDF do orçamento.")

    if st.session_state.get("perfil") in ["ADMINISTRADOR","GERENTE"]:
        df_img = load_products()
        if not df_img.empty:
            cimg1, cimg2 = st.columns([1,2])
            with cimg1:
                origem_img = st.selectbox(
                    "Origem",
                    ["TODAS","PNEUS COM NOTA","PNEUS SEM NOTA","DIESEL PNEUS"],
                    key="img_origem"
                )
            base_img = df_img if origem_img == "TODAS" else df_img[df_img["origem"] == origem_img]
            ...
            if busca_img:
                base_img = base_img[
                    base_img["descricao"].fillna("").astype(str).str.contains(busca_img, case=False, na=False) |
                    base_img["marca"].fillna("").astype(str).str.contains(busca_img, case=False, na=False)
                ]

            if not df_img.empty:
                total_img = len(df_img)
                confirmadas_img = int((df_img["imagem_status"] == "CONFIRMADA").sum())
                com_img = int(df_img["imagem_url"].fillna("").astype(str).str.strip().ne("").sum())
                m1, m2, m3 = st.columns(3)
                m1.metric("Produtos", total_img)
                m2.metric("Com imagem", com_img)
                m3.metric("Confirmadas", confirmadas_img)

            if not base_img.empty:
                opcoes_img = {
                    f'{r["descricao"]} | {r["origem"]}': int(r["id"])
                    for _, r in base_img.sort_values(["marca","descricao"]).iterrows()
                }

                escolha_img = st.selectbox(
                    "Selecione o produto",
                    list(opcoes_img.keys()),
                    key="img_produto"
                )
                pid_img = opcoes_img[escolha_img]
                row_img = df_img[df_img["id"] == pid_img].iloc[0]

                atual = "" if pd.isna(row_img.get("imagem_url")) else str(row_img.get("imagem_url") or "")
                fonte_atual = "" if pd.isna(row_img.get("imagem_fonte")) else str(row_img.get("imagem_fonte") or "")
                status_atual = str(row_img.get("imagem_status") or "PENDENTE")

                termo_busca = f'{row_img["marca"]} {row_img["descricao"]} pneu'
                termo_oficial = f'site oficial {row_img["marca"]} {row_img["descricao"]} pneu'
                google_img = "https://www.google.com/search?tbm=isch&q=" + quote_plus(termo_busca)
                google_oficial = "https://www.google.com/search?q=" + quote_plus(termo_oficial)

                st.markdown("##### 🔎 Buscar foto correta")
                st.caption(f"Pesquisa preparada: {termo_busca}")
                b1, b2 = st.columns(2)
                with b1:
                    st.link_button("🖼️ Abrir busca de imagens", google_img, use_container_width=True)
                with b2:
                    st.link_button("🏭 Procurar fonte oficial", google_oficial, use_container_width=True)

                st.caption("Confirme visualmente marca/modelo/medida e copie a URL pública da imagem abaixo.")

                url_img = st.text_input(
                    "URL pública da imagem escolhida",
                    value=atual,
                    placeholder="https://...",
                    key="img_url"
                ).strip()

                fonte_img = st.text_input(
                    "Fonte da imagem",
                    value=fonte_atual,
                    placeholder="Ex.: site oficial do fabricante / página do produto",
                    key="img_fonte"
                ).strip()

                status_opts = ["PENDENTE", "AUTO", "PROVÁVEL", "CONFIRMADA"]
                status_idx = status_opts.index(status_atual) if status_atual in status_opts else 0
                status_img = st.selectbox("Status da conferência", status_opts, index=status_idx, key="img_status")

                img_prev, info_prev = st.columns([1,2])
                with img_prev:
                    if url_img:
                        st.image(url_img, caption="Pré-visualização", width=260)
                    else:
                        st.info("Nenhuma imagem cadastrada.")
                with info_prev:
                    st.write(f"**Produto:** {row_img['descricao']}")
                    st.write(f"**Marca:** {row_img['marca']}")
                    st.write(f"**Origem:** {row_img['origem']}")
                    st.write(f"**Quantidade:** {int(row_img['quantidade'])}")

                if st.button("💾 Salvar imagem", key="salvar_img", use_container_width=True):
                    with engine.begin() as conn:
                        conn.execute(
                            text("""
                                UPDATE produtos
                                ...
                                WHERE id=:id
                            """),
                            {"url": url_img or None, "fonte": fonte_img or None,
                             "status": status_img, "id": int(pid_img)}
                        )
                    try:
                        registrar_auditoria(
                            "ALTERAÇÃO DE IMAGEM",
                            produto_id=int(pid_img),
                            descricao=str(row_img["descricao"]),
                            origem=str(row_img["origem"]),
                            observacao="Imagem do produto atualizada."
                        )
                    except Exception:
                        pass
                ...
                if atual and st.button("🗑️ Remover imagem", key="remover_img", use_container_width=True):
                    with engine.begin() as conn:
                        conn.execute(
                            text("UPDATE produtos SET imagem_url=NULL, imagem_fonte=NULL, imagem_status='PENDENTE' WHERE id=:id"),
                            {"id": int(pid_img)}
                        )
                    st.success("Imagem removida.")
                    st.rerun()
    else:
        st.info("Somente Administrador ou Gerente pode alterar imagens.")
...
def ficha_produto():
    ...
    img_url = "" if pd.isna(prod.get("imagem_url")) else str(prod.get("imagem_url") or "")
    if img_url:
        img_col, info_col = st.columns([1,2.2])
        with img_col:
            st.image(img_url, caption="Imagem do produto", use_container_width=True)
        with info_col:
            st.markdown("#### Informações do produto")
    else:
        st.info("🖼️ Este produto ainda não possui imagem cadastrada.")
...
menu_opcoes = ["Dashboard","Estoque","Movimentações","Ficha do Produto","Relatórios"]
