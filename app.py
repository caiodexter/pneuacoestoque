...
def estoque():
    ...
    st.markdown("---")
    st.subheader("🚀 Preenchimento automático de imagens")
    st.caption(
        "Busca gratuita. O sistema preenche somente produtos sem imagem e nunca substitui "
        "uma imagem marcada como CONFIRMADA. Faça em lotes para reduzir bloqueios temporários."
    )
    ...
    if st.session_state.get("perfil") in ["ADMINISTRADOR","GERENTE"]:
        ...
        pend_auto = df_auto[
            ...
            df_auto["imagem_status"].fillna("PENDENTE").astype(str).ne("CONFIRMADA")
        ].copy()

        a1, a2, a3 = st.columns(3)
        a1.metric("Sem imagem", len(pend_auto))
        lote_auto = a2.selectbox("Tamanho do lote", [5, 10, 20], index=1, key="lote_auto_img")
        min_score_auto = a3.number_input(
            "Confiança mínima", min_value=20, max_value=100, value=45, step=5,
            key="score_auto_img"
        )

        st.caption(
            "Recomendação inicial: lote de 10 e confiança mínima 45. "
            "Resultados abaixo do limite permanecem sem imagem para revisão manual."
        )

        if st.button(
            "🚀 Preencher próximo lote automaticamente",
            type="primary",
            use_container_width=True,
            disabled=pend_auto.empty,
            key="auto_img_lote"
        ):
            import time as _time
            lote_df = pend_auto.head(int(lote_auto))
            barra = st.progress(0)
            status_box = st.empty()
            ok_auto = 0
            revisar_auto = 0
            ...
            for idx_auto, (_, prod_auto) in enumerate(lote_df.iterrows(), start=1):
                status_box.write(f'Buscando {idx_auto}/{len(lote_df)} — {prod_auto["descricao"]}')
                try:
                    candidatos = buscar_imagem_automatica(prod_auto, max_results=8)
                    melhor = candidatos[0] if candidatos else None

                    if melhor and int(melhor["score"]) >= int(min_score_auto):
                        with engine.begin() as conn:
                            conn.execute(
                                text("""
                                    UPDATE produtos
                                    SET imagem_url=:url,
                                        ...
                                        imagem_status='AUTO'
                                    WHERE id=:id
                                      AND (imagem_url IS NULL OR TRIM(imagem_url)='')
                                      AND COALESCE(imagem_status,'PENDENTE') <> 'CONFIRMADA'
                                """),
                                {
                                    "url": melhor["url"],
                                    "fonte": melhor["fonte"] or melhor["titulo"],
                                    "id": int(prod_auto["id"])
                                }
                            )
                        ok_auto += 1
                ...
                barra.progress(idx_auto / len(lote_df))
                _time.sleep(1.2)

            status_box.empty()
            st.success(
                f"Lote concluído: {ok_auto} imagens cadastradas, "
                f"{revisar_auto} deixadas para revisão e {erros_auto} falhas de busca."
            )
            st.rerun()
